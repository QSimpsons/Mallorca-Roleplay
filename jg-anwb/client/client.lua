print('^2[jg-anwb] client.lua v7 geladen (progress-fix)^7')
...
-- Fallback als client/progress.lua niet geladen is (oude fxmanifest).
if type(SafeProgress) ~= 'function' then
	SafeProgress = function(data, cb)
		...
		if type(resource) == 'string' and resource ~= '' and GetResourceState(resource) == 'started' then
			local ok = pcall(function()
				exports[resource]:Progress(data, cb)
			end)
			if ok then
				return true
			end
		...
		CreateThread(function()
			...
			if lib and lib.progressBar then
				success = lib.progressBar({
					...
					disable = type(data.controlDisables) == 'table' and {
					...
					anim = type(data.animation) == 'table' and data.animation.animDict and {
