...
OpenSavedOutfitsMenu = function()
    ...
    if SafeCallExport('ox_appearance', 'showOutfitMenu') then
        return true
    end
    if SafeCallExport('illenium-appearance', 'openOutfitMenu') then
        return true
    end
    if SafeCallExport('fivem-appearance', 'openOutfitMenu') then
        return true
    end
    ...
    if ResourceStarted('ox_appearance') or ResourceStarted('illenium-appearance') or ResourceStarted('fivem-appearance') then
        return true
    end
...
OpenJobOutfitMenu = function(outfits)
    ...
    if SafeCallExport(jobsmenu, 'OpenOutfitMenu', outfits) then
        return true
    end
    if SafeCallExport(jobsmenu, 'openOutfitMenu', outfits) then
        return true
...
SafeToggleDuty = function()
    ...
    if SafeCallExport(jobsmenu, 'ToggleDuty', jobName) then
        return true
    end
    if SafeCallExport(jobsmenu, 'toggleDuty', jobName) then
        return true
...
SafeOpenManagement = function()
    ...
    if SafeCallExport(jobsmenu, 'OpenManagementMenu', jobName) then
        return true
    end
    if SafeCallExport(jobsmenu, 'openManagementMenu', jobName) then
        return true
    ...
    SafeNotify('error', 'Geen management-menu gevonden (jg-jobsmenu / esx_society).', 5000)
    return false
end
